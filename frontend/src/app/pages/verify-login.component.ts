...
@Component({
  ...
  template: `
    <h2 class="heading">Verify login OTP</h2>
    ...
    <form [formGroup]="form" (ngSubmit)="submit()">
      ...
      <div class="error" *ngIf="form.controls.otp.touched && form.controls.otp.invalid">
        OTP is required.
      </div>

      <button type="submit" [disabled]="form.invalid || loading()">Verify & Login</button>
    </form>
...
export class VerifyLoginComponent {
  ...
  submit() {
    ...
    this.auth.verifyLoginOtp({ email, otp }).subscribe({
      next: (res) => {
        this.auth.saveSession(res);
        this.message.set('Login successful');
        this.router.navigate(['/dashboard']);
      },
      error: (err) => this.error.set(err.error?.message || 'OTP verification failed'),
